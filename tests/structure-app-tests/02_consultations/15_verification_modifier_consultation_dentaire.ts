// tests/structure-app-tests/02_consultations/15_verification_modifier_consultation_dentaire.ts
...
import { ConsultationDentaireDataGenerator } from '../../../pages/structure/generator/consultation-dentaire-data-generator';
...
test('Modifier la première consultation dentaire d’un patient', async ({ page }) => {
    const loginPage = new LoginPage(page);
    const dashboardPage = new DashboardPage(page);
    const patientPage = new PatientPage(page);
    const consultationPage = new ConsultationDentairePage(page);
    const updatedData = ConsultationDentaireDataGenerator.generateUpdatedData();
    ...
    await test.step('Modifier les champs', async () => {
        await consultationPage.fillUpdatedDentaireForm(updatedData);
        await page.waitForTimeout(2000);
    });
