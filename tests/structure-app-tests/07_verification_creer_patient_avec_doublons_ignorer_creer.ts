...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Numéro Dossier' }).fill('P20266789');
  await page.locator('select[name="sex"]').selectOption('1');
  await page.locator('#lastName').click();
  await page.locator('#lastName').fill('JEAN ');
  ...
  await page.locator('#address').fill('medina');
  await page.locator('#street').click();
  await page.locator('#street').fill('90');
  ...
  await page.getByRole('option', { name: 'FATICK' }).click();
  ...
  await page.getByText('GABONAIS(E)').click();
  ...
  await page.locator('div').filter({ hasText: 'Il existe des duplicacats' }).nth(2).click();
  await page.getByRole('button', { name: 'Ignorer et créer le patient' }).click();
  await page.getByRole('button', { name: 'Confirmer' }).click();
});